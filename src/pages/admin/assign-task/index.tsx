...
// // on scroll data loading ( working code )
...
// // // How many dates to show around today on initial load
// // const INITIAL_BATCH = 2;
// // // How many dates to load when user scrolls up/down
...
// //   // Dates + pagination state
// //   const [availableDates, setAvailableDates] = useState<{ date: string; count: number }[]>([]);
// //   const [datePagination, setDatePagination] = useState<DatePaginationState>({});
// //   const [loadingDates, setLoadingDates] = useState(true);

// //   // Bi-directional lazy loading state
// //   // visibleRange.start and visibleRange.end are indices into availableDates array
// //   const [visibleRange, setVisibleRange] = useState<{ start: number; end: number }>({ start: 0, end: 0 });

// //   // Refs
// //   const todayRef = useRef<HTMLDivElement>(null);
// //   const loaderTopRef = useRef<HTMLDivElement>(null);
// //   const loaderBottomRef = useRef<HTMLDivElement>(null);
// //   const todayIndexRef = useRef<number>(-1);
// //   const scrolledToTodayRef = useRef<boolean>(false); // prevent multiple auto-scrolls
...
// //   const selectedCompanyId = useMemo(() => {
// //     if (hasBothCompanies) return companyTabs[companyTab]?.companyId;
// //     if (hasSakshi) return getCompanyWisePermission(5);
// //     if (hasQP) return getCompanyWisePermission(6);
// //     return null;
// //   }, [hasBothCompanies, companyTab, hasSakshi, hasQP, companyTabs]);
...
// //   // ─── Helper: build common query params ────────────────────────────────────
...
// //         if (filters[key] && filters[key].length > 0) {
// //           const backendField = fieldMap[key] || key;
// //           queryParams[backendField] =
// //             filters[key].length > 1 ? filters[key].join(",") : filters[key][0];
// //         }
// //       });
// //     }
...
// //     async (date: string, page: number, itemsPerPage: number) => {
// //       // Mark as loading
// //       setDatePagination((prev) => ({
// //         ...prev,
...
// //   // ─── Fetch all dates (counts only) ────────────────────────────────────────
// //   const fetchDates = useCallback(
// //     async (sd: Date | null, ed: Date | null) => {
// //       setLoadingDates(true);
// //       scrolledToTodayRef.current = false; // reset scroll flag on every fresh fetch
...
// //         // Override dates with fresh params passed directly
// //         if (sd && ed) {
// //           queryParams.startDate = sd.toISOString();
// //           queryParams.endDate = ed.toISOString();
// //         } else {
...
// //           // Initialize pagination shell (no data yet)
// //           const newPagination: DatePaginationState = {};
...
// //           // ── Find today's index ──
// //           // fetchDates ke andar — yeh exact hona chahiye
// //           const todayMidnight = new Date();
// //           todayMidnight.setHours(0, 0, 0, 0);

// //           let foundIndex = sortedDates.findIndex((d) => {
// //             const dt = new Date(d.date); // ✅ ISO parse
// //             dt.setHours(0, 0, 0, 0);
// //             return dt.getTime() === todayMidnight.getTime(); // exact today match
// //           });

// //           // Agar today nahi mila → closest past date (descending mein pehla jo <= today ho)
// //           if (foundIndex === -1) {
// //             foundIndex = sortedDates.findIndex((d) => {
// //               const dt = new Date(d.date);
// //               dt.setHours(0, 0, 0, 0);
// //               return dt.getTime() <= todayMidnight.getTime(); // ✅ past ya today
// //             });
// //           }

// //           if (foundIndex === -1) foundIndex = 0; // complete fallback

// //           todayIndexRef.current = foundIndex;

// //           // ── Set visible range around today ──
// //           const rangeStart = Math.max(0, foundIndex - INITIAL_BATCH);
// //           const rangeEnd = Math.min(sortedDates.length - 1, foundIndex + INITIAL_BATCH);
// //           setVisibleRange({ start: rangeStart, end: rangeEnd });

// //           // ── Fetch tasks only for visible range ──
// //           sortedDates.slice(rangeStart, rangeEnd + 1).forEach((dateInfo) => {
// //             fetchTasksForDate(dateInfo.date, 1, ITEMS_PER_PAGE);
// //           });
// //         }
// //       } catch (error) {
...
// //       }
// //     },
// //     [buildBaseQueryParams, fetchTasksForDate]
// //   );

// //   // ─── Auto-scroll to today once dates + tasks are loaded ───────────────────
// //   useEffect(() => {
// //     if (
// //       !loadingDates &&
// //       availableDates.length > 0 &&
...
// //   // ─── Intersection observers for bi-directional lazy loading ───────────────
// //   useEffect(() => {
// //     if (availableDates.length === 0) return;

// //     // TOP sentinel — user scrolled up
// //     // TOP sentinel observer — user scrolled up
// //     const topObserver = new IntersectionObserver(
// //       (entries) => {
// //         if (entries[0].isIntersecting && visibleRange.start > 0) {
// //           const newStart = Math.max(0, visibleRange.start - LOAD_MORE_COUNT);
// //           const container = scrollContainerRef.current;

// //           // ✅ Naya content add hone se PEHLE current scroll height save karo
// //           const scrollHeightBefore = container?.scrollHeight ?? 0;
// //           const scrollTopBefore = container?.scrollTop ?? 0;

// //           availableDates.slice(newStart, visibleRange.start).forEach((dateInfo) => {
// //             const existing = datePagination[dateInfo.date];
// //             if (!existing?.data?.length && !existing?.loading) {
// //               fetchTasksForDate(dateInfo.date, 1, ITEMS_PER_PAGE);
// //             }
// //           });
...
// //           // ✅ DOM update hone ke baad scroll position restore karo
...
// //     // BOTTOM sentinel — user scrolled down
// //     const bottomObserver = new IntersectionObserver(
// //       (entries) => {
// //         if (entries[0].isIntersecting && visibleRange.end < availableDates.length - 1) {
// //           const newEnd = Math.min(availableDates.length - 1, visibleRange.end + LOAD_MORE_COUNT);

// //           // Fetch newly exposed dates
// //           availableDates.slice(visibleRange.end + 1, newEnd + 1).forEach((dateInfo) => {
// //             const existing = datePagination[dateInfo.date];
// //             if (!existing?.data?.length && !existing?.loading) {
// //               fetchTasksForDate(dateInfo.date, 1, ITEMS_PER_PAGE);
// //             }
...
// //     if (loaderTopRef.current) topObserver.observe(loaderTopRef.current);
// //     if (loaderBottomRef.current) bottomObserver.observe(loaderBottomRef.current);

// //     return () => {
// //       topObserver.disconnect();
// //       bottomObserver.disconnect();
// //     };
// //   }, [availableDates, visibleRange, datePagination, fetchTasksForDate]);

// //   // ─── Re-fetch when tabs / company change ──────────────────────────────────
// //   useEffect(() => {
// //     if ((canViewGlobal || canViewOwn) && router.isReady && selectedCompanyId) {
// //       setDatePagination({});
// //       setAvailableDates([]);
...
// //   // ─── Query param driven initial state ─────────────────────────────────────
// //   useEffect(() => {
// //     if (st) {
// //       const start = new Date(st as string);
// //       setStartDate(start);
...
// //           filterField: apiField,
// //         });
// //         if (response?.data) setFilterOptions(response.data || []);
// //         else {
// //           toast.error("Failed to load filter options");
// //           setFilterOptions([]);
// //         }
...
// //     };
// //     fetchFilterOptions();
// //   }, [selectedFilterField, filters, selectedCompanyId, selectedStatus, companyTab]);

// //   // ─── Handlers ─────────────────────────────────────────────────────────────
// //   const handleSearch = (start: Date | null, end: Date | null) => {
// //     setAppliedSearchQuery(searchQuery);
// //     setAppliedStartDate(start);
// //     setAppliedEndDate(end);
// //     setDatePagination({});
...
// //       setDatePagination((prev) => ({
// //         ...prev,
// //         [date]: { ...prev[date], currentPage: page },
// //       }));
// //     }
// //   };

// //   const handleSelectTask = (task: Task) => {
// //     setSelectedTaskIds((prevSelected: any) => {
// //       const isSelected = prevSelected.some((t: any) => t?._id === task?._id);
// //       if (isSelected) return prevSelected.filter((t: any) => t?._id !== task?._id);
// //       return [...prevSelected, task];
// //     });
// //   };
...
// //       datePagination[availableDates[0]?.date]?.data?.map((task) => task._id) || [];
// //     if (selectedTaskIds.length === currentTaskIds.length) setSelectedTaskIds([]);
// //     else setSelectedTaskIds(currentTaskIds);
// //   };
...
// //       try {
// //         await assignTaskService.deleteAssignTask(id);
// //         Swal.fire({ title: "Deleted!", text: "Task deleted successfully", icon: "success", confirmButtonColor: "#7F56D9" });
// //         setDatePagination({});
// //         fetchDates(null, null);
// //       } catch (err: any) {
// //         Swal.fire({ title: "Error!", text: err.message || "Failed to delete task", icon: "error", confirmButtonColor: "#7F56D9" });
// //       }
// //     }
// //   };
...
// //       try {
// //         await assignTaskService.bulkDeleteAssignTasks(selectedTaskIds);
// //         Swal.fire({ title: "Deleted!", text: `${selectedTaskIds.length} task(s) deleted successfully`, icon: "success", confirmButtonColor: "#7F56D9" });
// //         setDatePagination({});
// //         fetchDates(null, null);
// //         setSelectedTaskIds([]);
// //       } catch (err: any) {
// //         Swal.fire({ title: "Error!", text: err.message || "Failed to delete tasks", icon: "error", confirmButtonColor: "#7F56D9" });
// //       }
// //     }
// //   };
...
// //   // ─── Helpers ──────────────────────────────────────────────────────────────
// //   // ✅ AFTER — ISO string directly parse karo (availableDates ka format)
// //   const isToday = (dateString: string): boolean => {
...
// //   const defaultReasons = ["Delivery", "Get Payment", "Get Visit", "Order", "Complain", "Sample Approval", "Other"];
...
// //     contactPerson: task.partyName?.contactPerson || task.partyName?.ownerName || "N/A",
// //     area: (task.partyName?.address?.area as any)?.area || "N/A",
// //     mobile: task.partyName?.personMobileNo || (task.partyName as any)?.ownerMobile || "N/A",
// //     partyTag: task.partyName?.partyTag?.substring(0, 4) || "N/A",
// //     remarks: task.remarks || "N/A",
// //     assignBy: task.partyName?.createdBy
// //       ? `${task.partyName.createdBy.firstName} ${task.partyName.createdBy.lastName}`
// //       : "Unknown",
// //     feedback: (task as any).feedback || "N/A",
// //     AssignTo: task.assignTo,
// //     assignTo: task.assignTo
// //       ? `${task.assignTo.firstName} ${task.assignTo.lastName}`
// //       : "Unassigned",
...
// //     highlightYellow:
// //       !defaultReasons.includes(task.reasonForVisit) &&
// //       (task.assignTo as any)?.role?.roleName?.toLowerCase() === "driver",
// //   });
...
// //         <TableCell sx={getCellSx()}>
// //           <Box display="flex" alignItems="center" gap={1}>
// //             <Avatar sx={{ width: 32, height: 32 }} src={row.company.avatar} alt={row.company.name} />
// //             <Box>
// //               {row.isRescheduledTask && (
// //                 <Tooltip title={`Rescheduled from ${row.originalTaskDate}`}>
// //                   <ThemeChip
...
// //                     color="warning"
// //                     size="small"
// //                     sx={{ mt: 0.5, background: "#FFFAEB", color: "#B54708", fontSize: 11, height: 20 }}
// //                   />
// //                 </Tooltip>
// //               )}
// //             </Box>
...
// //         <TableCell
// //           sx={getCellSx({ cursor: "pointer", fontSize: 14 })}
// //           onClick={() => router.push(`/admin/assign-task/view-task/${row.partyId}?taskId=${row?._id}`)}
// //         >
// //           {row.party}
// //         </TableCell>
// //         <TableCell sx={getCellSx({ fontSize: 14 })}>
...
// //         </TableCell>
// //         <TableCell sx={getCellSx()}>
// //           <Typography sx={{ fontSize: 14 }} title={row.feedback} noWrap>
// //             {row.feedback && row.feedback.length > 10 ? `${row.feedback.substring(0, 10)}...` : row.feedback}
// //           </Typography>
// //         </TableCell>
// //         <TableCell sx={getCellSx({ fontSize: 14 })}>{getFirstFourChars(row.status)}</TableCell>
// //         <TableCell sx={getCellSx({ fontSize: 14 })}>{row.assignBy}</TableCell>
...
// //           <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
// //             {canedit && (
// //               <IconButton color="primary" onClick={() => { setRowData(row); handleEdit(row.id); }}>
// //                 <EditIcon />
// //               </IconButton>
// //             )}
// //             {candelete && (
...
// //       {!hasBothCompanies && selectedCompanyId && (
// //         <Box sx={{ mb: 2, p: 2, backgroundColor: "primary.light", color: "primary.contrastText", borderRadius: 1 }}>
// //           <Typography variant="h6">Showing tasks for: {hasSakshi ? "Sakshi" : "QP"}</Typography>
// //         </Box>
// //       )}
...
// //           <ThemeButton onClick={() => handleSearch(startDate, endDate)}>Search</ThemeButton>
// //           <ThemeButton onClick={handleClear} color="secondary">Clear</ThemeButton>

// //           <FilterDropdown
// //             filterOptions={
// //               canViewOwn && !canViewGlobal
// //                 ? ["Date", "Party", "Unit No", "Market Name", "Area", "Mobile No.", "Reason to Visit", "Assign By"]
// //                 : ["Date", "Party", "Unit No", "Market Name", "Area", "Mobile No.", "Reason to Visit", "Assign By", "Assign To"]
// //             }
// //             uniqueValues={filterOptions}
...
// //               <path fill="#667085" d="M224 136V0H24C10.7 0 0 10.7 0 24v464c0 13.3 10.7 24 24 24h336c13.3 0 24-10.7 24-24V160H248c-13.2 0-24-10.8-24-24zm60.1 106.5L224 336l60.1 93.5c5.1 8-.6 18.5-10.1 18.5h-34.9c-4.4 0-8.5-2.4-10.6-6.3C208.9 405.5 192 373 192 373c-6.4 14.8-10 20-36.6 68.8-2.1 3.9-6.1 6.3-10.5 6.3H110c-9.5 0-15.2-10.5-10.1-18.5l60.3-93.5-60.3-93.5c-5.2-8 .6-18.5 10.1-18.5h34.8c4.4 0 8.5 2.4 10.6 6.3 26.1 48.8 20 33.6 36.6 68.5 0 0 6.1-11.7 36.6-68.5 2.1-3.9 6.2-6.3 10.6-6.3H274c9.5-.1 15.2 10.4 10.1 18.4zM384 121.9v6.1H256V0h6.1c6.4 0 12.5 2.5 17 7l97.9 98c4.5 4.5 7 10.6 7 16.9z" />
// //             </svg>
// //           </IconButton>

// //           {cancreate && (
// //             <ThemeButton onClick={() => { setEditId(null); setTempEditId(null); setOpen(true); }}>
// //               + Assign New Task
// //             </ThemeButton>
// //           )}

// //           {candelete && selectedTaskIds.length > 0 && (
// //             <ThemeButton onClick={handleBulkDelete} color="error">
// //               Delete Selected ({selectedTaskIds.length})
// //             </ThemeButton>
// //           )}
// //         </Box>
...
// //         ) : (
// //           <>
// //             {/* ── TOP sentinel: triggers loading older dates when user scrolls up ── */}
// //             {visibleRange.start > 0 && (
// //               <Box
// //                 ref={loaderTopRef}
// //                 sx={{ display: "flex", justifyContent: "center", alignItems: "center", py: 2, gap: 1 }}
// //               >
// //                 <Loader />
// //                 <Typography fontSize={13} color="text.secondary">Loading earlier dates…</Typography>
// //               </Box>
// //             )}

// //             {/* ── Render only dates in visible range ── */}
// //             {availableDates
...
// //                   data: [],
// //                 };
// //                 const { currentPage, loading, data } = pagination;

// //                 return (
// //                   <Box
// //                     key={date}
// //                     // ✅ todayRef attaches here — auto-scroll target
// //                     ref={isToday(date) ? todayRef : null}
// //                     mb={4}
// //                     sx={{
// //                       backgroundColor: isToday(date) ? "#a0d8b4ff" : "transparent",
...
// //                     <Typography variant="subtitle1" fontWeight={600}>
// //                       Task -{" "}
// //                       <span style={{ color: "red" }}>{formatDateToDDMMYYYY(date)}</span>
// //                       {isToday(date) && (
// //                         <ThemeChip
// //                           label="Today"
// //                           color="success"
...
// //                         count={datePagination[date].totalItems}
// //                         page={currentPage}
// //                         handlePageChange={(d, page) => handlePageChange(d, page)}
// //                         date={date}
// //                         showHeaderCheckbox={false}
// //                         onSelectAll={(event: React.ChangeEvent<HTMLInputElement>) => handleSelectAllTasks(event)}
// //                         onSelectRow={(id: string) => handleSelectTask(id)}
...
// //             {/* ── BOTTOM sentinel: triggers loading newer dates when user scrolls down ── */}
// //             {visibleRange.end < availableDates.length - 1 ? (
// //               <Box
// //                 ref={loaderBottomRef}
// //                 sx={{
// //                   display: "flex",
...
// //                 width="100%"
// //               >
// //                 ✓ All {availableDates.length} date{availableDates.length !== 1 ? "s" : ""} loaded
// //               </Typography>
// //             )}
// //           </>
// //         )}
...
// //         toggleQpDialog={toggleQpDialog}
// //         taskId={editId}
// //         refreshData={() => { setDatePagination({}); fetchDates(null, null); }}
// //         rowData={rowData}
// //         companyTab={companyTab}
// //         company={companies?.find((item) => item.companyName === StaticCompanyOptions[companyTab])}
// //       />
...
// //           company={companies.find((item) => item.companyName === StaticCompanyOptions[1])?._id}
// //           open={qpDialog}
// //           onClose={() => { toggleQpDialog(); setTempEditId(null); }}
// //           refreshData={() => { setDatePagination({}); fetchDates(null, null); }}
// //           party={assignTasks.find((item) => item._id === tempEditId)?.partyName?._id}
// //         />
// //       )}
...
// //           company={companies.find((item) => item.companyName === StaticCompanyOptions[0])?._id}
// //           open={scDialog}
// //           onClose={() => { toggleScDialog(); setTempEditId(null); }}
// //           party={assignTasks.find((item) => item._id === tempEditId)?.partyName?._id}
// //         />
// //       )}
// //     </>
...
const AddQPOrderDialog = dynamic(
  () => import("@/component/allorderdailog/QpOrderDialog"),
  { ssr: false }
);

const AddSakhiOrderDialog = dynamic(
  () => import("@/component/allorderdailog"),
  { ssr: false }
);

const AssignTaskDialog = dynamic(
