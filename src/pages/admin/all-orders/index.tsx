...
//   const [selectedOrderForDelete, setSelectedOrderForDelete] = useState<OrderRow | null>(null)
...
//   const handleDeleteClick = (rowData: OrderRow) => {
//     setSelectedOrderForDelete(rowData);
...
//     try {
//       await orderService.deleteOrder(selectedOrderForDelete._id);
...
const AllOrdersPage = () => {
  ...
  // FIX 2: Single loadOrders, no duplicate useEffect
  const loadOrders = useCallback(async () => {
  ...
  const loadFilterOptions = async (field: string) => {
    ...
    try {
      ...
      const response = await orderService.searchFilterOptions(field, "", filterPayload);
      if (response.success && response.data) {
        setFilterOptionsData(prev => ({ ...prev, [field]: response.data || [] }));
      }
    } catch (error: any) {
      toast.error(`Failed to load filter options for ${field}`);
    } finally {
  ...
  const handleFiltersChange = useCallback((newFilters: { [key: string]: string[] }) => {
    setCurrentFilterState((prev: any) => ({ ...prev, filters: newFilters, page: 1 }));
  }, []);

  // FIX 2: Only ONE useEffect to load orders (removed duplicate)
  useEffect(() => {
    loadOrders()
  }, [loadOrders]);
  ...
  const handleFollowUpSuccess = () => { loadOrders(); };
  ...
  const handleExcelDownload = async () => {
    ...
    if (!orders || orders.length === 0) { toast.info("No orders available to export."); return; }
    setDownloadLoading(true);
    try {
      const payload = {
        ...currentFilterState,
  ...
  const handleDownloadPendingApprovalExcel = async () => {
    if (downloadingPendingApproval) return;
    setDownloadingPendingApproval(true);
    try {
      const payload = {
        startDate: currentFilterState.startDate || undefined,
  ...
  const getRouteByStatus = (row: OrderRow): string => {
    const { status, designer, printer, binder, bookletBinder } = row;
    switch (status) {
      case "Received": return `/admin/all-orders/view?id=${row._id}`;
      case "Designer": return `/admin/all-orders/view/designer?id=${row._id}`;
      case "Printer": return `/admin/all-orders/view/printers?id=${row._id}`;
      case "Binder": return `/admin/all-orders/view/binder?id=${row._id}`;
      case "Booklet & Folder Binder": return `/admin/all-orders/view/booklet-folder?id=${row._id}`;
      case "Delivery": return `/admin/all-orders/view/dilevery?id=${row._id}`;
      case "Hold":
        if (bookletBinder) return `/admin/all-orders/view/booklet-folder?id=${row._id}`;
        else if (binder) return `/admin/all-orders/view/binder?id=${row._id}`;
        else if (printer) return `/admin/all-orders/view/printers?id=${row._id}`;
        else if (designer) return `/admin/all-orders/view/designer?id=${row._id}`;
        else return `/admin/all-orders/view?id=${row._id}`;
      default: return `/admin/all-orders/view?id=${row._id}`;
    }
  };
  ...
  const handleDownloadInvoice = (row: OrderRow) => {
    try {
      if (!row?.party?.address) { toast.error("Address data is missing"); return; }
      const latestQuotation = row?.quotation?.[row?.quotation?.length - 1];
      const quantity = Number(row?.qty) || 0;
      const unitPrice = Number(latestQuotation?.unitPrice) || 0;
      const subtotal = quantity * unitPrice;
      ...
      const formData = {
        ...
        GSTNo: row?.party?.GSTNo || "N/A",
        servicePerformance: row?.productItem?.itemName || "N/A",
        quantity, unitPrice, total: subtotal, finalAmount: totalAmount,
        applyGST, gstPercentage, daysAfterConfirmation: row?.daysAfterConfirmation || 0,
      }
      generateInvoicePDF(formData)
      toast.success("Quotation downloaded successfully")
    } catch (error) {
  ...
  const handleProformaDownload = (row: OrderRow) => {
    try {
      ...
      const formData = {
        ...
        ownerMobileNo: row?.party?.ownerMobileNo || "",
        partyName: row?.party?.partyName || "N/A",
        addressName: `${row?.party?.address?.unitNo || ""}, ${typeof row?.party?.address?.marketName === 'object' ? row?.party?.address?.marketName?.marketName : row?.party?.address?.marketName || ""}, ${typeof row?.party?.address?.area === 'object' ? row?.party?.address?.area?.area : row?.party?.address?.area || ""} - ${row?.party?.address?.pincode?.pincode || ""}`,
        GSTNo: row?.party?.GSTNo || "N/A",
        servicePerformance: row?.productItem?.itemName || "N/A",
        quantity: row?.qty || 0,
        unitPrice: row?.unitPrice || 0,
  ...
  const StatusBadge = ({ row }: { row: OrderRow }) => {
    ...
    if (isHold) {
      return (
        <Box display="flex" alignItems="center" gap={1}>
          <Typography sx={{ fontSize: "13px", fontWeight: 500, color: "#374151" }}>{text}</Typography>
          <Box sx={{ backgroundColor: "#DC2626", color: "#FFFFFF", fontSize: "10px", fontWeight: 600, borderRadius: "4px", px: 1, py: 0.25, textTransform: "uppercase" }}>
            HOLD
          </Box>
        </Box>
      );
    }
    return <Typography sx={{ fontSize: "13px", fontWeight: 500, color: "#374151" }}>{text}</Typography>;
  };

  const getAvatarUrl = (row: OrderRow) => {
    if (row.companyName && (row.companyName as any).avatar) return (row.companyName as any).avatar;
  };
  ...
  const renderRow = (row: OrderRow, index: number) => {
    let counter = 0
    if (row.designerNotificationUnread === true) counter++
    if (row.printerNotificationUnread === true) counter++
    if (row.binderNotificationUnread === true) counter++
    if (row.bookletBinderNotificationUnread === true) counter++
    ...
    return (
      <>
        ...
        <TableCell>
          <Box display="flex" alignItems="center" gap={2}>
            <Avatar src={getAvatarUrl(row)} sx={{ width: 32, height: 32 }} alt={row.companyName?.companyName || "Company"} />
          </Box>
        </TableCell>
        <TableCell>
          <Typography fontSize="14px" color="#6B7280">{formatDateToDDMMYYYY(row.createdAt)}</Typography>
        </TableCell>
        <TableCell>
          <Typography fontSize="14px" color="#6B7280">{row.party?.partyName || "N/A"}</Typography>
        </TableCell>
        <TableCell>
          <Typography fontSize="14px" color="#6B7280">{row.productItem?.itemName || "N/A"}</Typography>
        </TableCell>
        <TableCell>
          <Typography fontSize="14px" color="#6B7280">
            {typeof row?.size === "object" ? row.size?.size : row?.size || "N/A"}
        ...
        <TableCell>
          <Typography sx={{ fontSize: 14, color: "text.secondary" }} title={row.remarks} noWrap>
            {row.remarks && row.remarks.length > 10 ? `${row.remarks.substring(0, 13)}...` : row.remarks}
          </Typography>
        </TableCell>
        <TableCell>
          <Typography fontSize="14px" color="#6B7280">
        ...
        <TableCell>
          ...
          >
            <StatusBadge row={row} />
            {counter > 0 && (
              <Badge color="primary" sx={{ mt: 1.6 }} badgeContent={counter}>{counter}</Badge>
            )}
            <FaChevronRight style={{ fontSize: 14, color: "#9CA3AF", marginLeft: 8 }} />
          </Box>
        </TableCell>
        <TableCell>
          <Box display="flex" alignItems="center" gap={1}>
            {row.followUp?.staff ? (
              <>
                <Avatar src={row.followUp.staff.avatar} alt={`${row.followUp.staff.firstName} ${row.followUp.staff.lastName}`} sx={{ width: 24, height: 24 }} />
                <Typography fontSize="12px" color="#6B7280">
                  {row.followUp.staff.firstName} {row.followUp.staff.lastName}
                </Typography>
                <Box display="flex" flexDirection="column" gap={0.5}>
                  <Box sx={{
                    backgroundColor: row.followUp.status === "Completed" ? "#10B981" : row.followUp.status === "In Progress" ? "#3B82F6" : row.followUp.status === "Cancelled" ? "#EF4444" : row.followUp.status === "Rescheduled" ? "#8B5CF6" : "#F59E0B",
                    color: "#FFFFFF", fontSize: "10px", fontWeight: 600, borderRadius: "4px", px: 0.75, py: 0.25, textTransform: "uppercase", width: "fit-content",
                  }}>
                    {row.followUp?.taskId?.status || ""}
                  </Box>
                  {row.followUp?.taskId?.status === "Rescheduled" && row.followUp?.taskId?.rescheduleDate && (
                    <Typography fontSize="10px" color="#8B5CF6" sx={{ backgroundColor: "#EDE9FE", borderRadius: "4px", px: 0.75, py: 0.25, fontWeight: 500, width: "fit-content" }}>
                      {moment(row.followUp.taskId.rescheduleDate).format("DD-MM-YYYY")}
                    </Typography>
                  )}
                </Box>
              </>
            ) : (
              <Typography fontSize="12px" color="#9CA3AF">Not Assigned</Typography>
            )}
          </Box>
        </TableCell>
        <TableCell>
          <Box display="flex" gap={1}>
            <Button variant="outlined" size="small" onClick={() => handleFollowUpClick(row)} sx={{ fontSize: "11px", textTransform: "none" }}>
              {row.followUp?.staff ? "Reassign" : "Follow Up"}
            </Button>
            <Button variant="outlined" size="small" onClick={() => handleProformaDownload(row)} sx={{ fontSize: "12px", textTransform: "none" }}>
              Proforma
            </Button>
            {row.status !== "Cancelled" && (
              <Button variant="outlined" size="small" color="error" onClick={() => handleCancelClick(row)} sx={{ fontSize: "12px", textTransform: "none" }}>
                Cancel
              </Button>
            )}
            <Button variant="outlined" size="small" color="error" onClick={() => handleDeleteClick(row)} sx={{ fontSize: "12px", textTransform: "none" }}>
              Delete
            </Button>
          </Box>
        </TableCell>
  ...
  // Shared dialogs used in both Sakshi and QP content
  const renderSharedDialogs = () => (
    <>
      <FollowUpDialog
        open={followUpDialogOpen}
        onClose={() => { setFollowUpDialogOpen(false); setSelectedOrderForFollowUp(null); }}
        order={selectedOrderForFollowUp}
        onSuccess={handleFollowUpSuccess}
      />
      <CancelOrderDialog
  ...
  // Shared download buttons toolbar
  const renderToolbar = () => (
    <Box sx={{ display: "flex", alignItems: "center", gap: 2, mt: 2 }}>
      <IconButton
        onClick={handleExcelDownload}
        disabled={downloadLoading || loading || isLoadingData}
        sx={{ border: "1px solid #D0D5DD", borderRadius: 2, p: 1, color: downloadLoading ? "#9CA3AF" : "#667085", display: "flex", alignItems: "center", cursor: downloadLoading ? 'not-allowed' : 'pointer' }}
        title={downloadLoading ? "Downloading..." : "Download as Excel"}
      >
        {downloadLoading ? <CircularProgress size={16} color="inherit" /> : (
          <svg xmlns="http://www.w3.org/2000/svg" height="16" width="16" viewBox="0 0 384 512">
            <path fill="#667085" d="M224 136V0H24C10.7 0 0 10.7 0 24v464c13.3 0 24 10.7 24 24h336c13.3 0 24-10.7 24-24V160H248c-13.2 0-24-10.8-24-24zm60.1 106.5L224 336l60.1 93.5c5.1 8-.6 18.5-10.1 18.5h-34.9c-4.4 0-8.5-2.4-10.6-6.3C208.9 405.5 192 373 192 373c-6.4 14.8-10 20-36.6 68.8-2.1 3.9-6.1 6.3-10.5 6.3H110c-9.5 0-15.2-10.5-10.1-18.5l60.3-93.5-60.3-93.5c-5.2-8 .6-18.5 10.1-18.5h34.8c4.4 0 8.5 2.4 10.6 6.3 26.1 48.8 20 33.6 36.6 68.5 0 0 6.1-11.7 36.6-68.5 2.1-3.9 6.2-6.3 10.6-6.3H274c9.5-.1 15.2 10.4 10.1 18.4zM384 121.9v6.1H256V0h6.1c6.4 0 12.5 2.5 17 7l97.9 98c4.5 4.5 7 10.6 7 16.9z"/>
          </svg>
        )}
        <Typography fontSize={12} sx={{ ml: 1 }}>Download orders</Typography>
      </IconButton>
      ...
      <IconButton
        onClick={handleDownloadPendingApprovalExcel}
        disabled={downloadingPendingApproval || loading || isLoadingData}
        sx={{ border: "1px solid #D0D5DD", borderRadius: 2, p: 1, color: downloadingPendingApproval ? "#9CA3AF" : "#667085", display: "flex", alignItems: "center", cursor: downloadingPendingApproval ? 'not-allowed' : 'pointer' }}
        title={downloadingPendingApproval ? "Downloading..." : "Download Pending Approval Orders"}
      >
        {downloadingPendingApproval ? <CircularProgress size={16} color="inherit" /> : (
          <svg xmlns="http://www.w3.org/2000/svg" height="16" width="16" viewBox="0 0 384 512">
            <path fill="#667085" d="M224 136V0H24C10.7 0 0 10.7 0 24v464c13.3 0 24 10.7 24 24h336c13.3 0 24-10.7 24-24V160H248c-13.2 0-24-10.8-24-24zm60.1 106.5L224 336l60.1 93.5c5.1 8-.6 18.5-10.1 18.5h-34.9c-4.4 0-8.5-2.4-10.6-6.3C208.9 405.5 192 373 192 373c-6.4 14.8-10 20-36.6 68.8-2.1 3.9-6.1 6.3-10.5 6.3H110c-9.5 0-15.2-10.5-10.1-18.5l60.3-93.5-60.3-93.5c-5.2-8 .6-18.5 10.1-18.5h34.8c4.4 0 8.5 2.4 10.6 6.3 26.1 48.8 20 33.6 36.6 68.5 0 0 6.1-11.7 36.6-68.5 2.1-3.9 6.2-6.3 10.6-6.3H274c9.5-.1 15.2 10.4 10.1 18.4zM384 121.9v6.1H256V0h6.1c6.4 0 12.5 2.5 17 7l97.9 98c4.5 4.5 7 10.6 7 16.9z"/>
          </svg>
        )}
      ...
      <IconButton
        onClick={handleExportPendingClientApproval}
        sx={{ border: "1px solid #D0D5DD", borderRadius: 2, p: 1, color: "#667085", display: "flex", alignItems: "center" }}
        title="Download Pending Approval Design Orders"
      >
        <svg xmlns="http://www.w3.org/2000/svg" height="16" width="16" viewBox="0 0 384 512">
          <path fill="#667085" d="M224 136V0H24C10.7 0 0 10.7 0 24v464c13.3 0 24 10.7 24 24h336c13.3 0 24-10.7 24-24V160H248c-13.2 0-24-10.8-24-24zm60.1 106.5L224 336l60.1 93.5c5.1 8-.6 18.5-10.1 18.5h-34.9c-4.4 0-8.5-2.4-10.6-6.3C208.9 405.5 192 373 192 373c-6.4 14.8-10 20-36.6 68.8-2.1 3.9-6.1 6.3-10.5 6.3H110c-9.5 0-15.2-10.5-10.1-18.5l60.3-93.5-60.3-93.5c-5.2-8 .6-18.5 10.1-18.5h34.8c4.4 0 8.5 2.4 10.6 6.3 26.1 48.8 20 33.6 36.6 68.5 0 0 6.1-11.7 36.6-68.5 2.1-3.9 6.2-6.3 10.6-6.3H274c9.5-.1 15.2 10.4 10.1 18.4zM384 121.9v6.1H256V0h6.1c6.4 0 12.5 2.5 17 7l97.9 98c4.5 4.5 7 10.6 7 16.9z"/>
        </svg>
  ...
  const renderTable = () => (
    (loading || isLoadingData) && orders.length === 0 ? (
      ...
      <CustomTable2
        ...
        defaultFilter={defaultOrderFilter}
        currentFilterState={currentFilterState}
        currentPage={currentFilterState.page}
        onPageChange={(newPage: number) => {
          setCurrentFilterState((prev: any) => ({ ...prev, page: newPage }))
        }}
        renderRow={renderRow}
  ...
  // FIX 1: renderSakshiContent defined properly
  const renderSakshiContent = () => (
    <>
      ...
      {complainOpen && selectedOrderForComplain && (
        <ComplainDialogue
          company={{ _id: selectedOrderForComplain.companyName._id, companyName: selectedOrderForComplain.companyName.companyName }}
          open={complainOpen}
          onClose={() => { setComplainOpen(false); setSelectedOrderForComplain(null); }}
          selectedOrderData={selectedOrderForComplain}
        />
      )}
      {renderSharedDialogs()}
    </>
  ...
  // FIX 1: renderQpContent now exists
  const renderQpContent = () => (
    <>
      <Box sx={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 2, mb: 2 }}>
        {renderToolbar()}
        <ThemeButton onClick={() => setOpen(true)}>+ Add New Order</ThemeButton>
  ...
  const renderContent = () => {
    ...
    if (hasSakshiPermission) return renderSakshiContent();
    if (hasQpPermission) return renderQpContent();
    return (
      <Box display="flex" justifyContent="center" alignItems="center" height="200px">
        <Typography variant="h6" color="error">You don't have permission to view this page.</Typography>
      </Box>
    );
  };

  // FIX 2: return is now correctly INSIDE the component
  return <>{renderContent()}</>;
}
