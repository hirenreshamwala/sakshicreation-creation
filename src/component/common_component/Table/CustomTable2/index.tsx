...
interface OrderTableProps<T> {
  ...
  onFilterFieldSelect?: (field: string | null) => void;
  onFieldOpen?: (field: string) => Promise<void>;
  selectedFilterField?: string | null;
  filterOptionsData?: { [key: string]: string[] };
  loadingFilterOptions?: boolean;
  onFiltersChange?: (filters: { [key: string]: string[] }) => void;
}

const CustomTable2 = <T extends { id: string; _id?: string; lastStatusChangeDate?: string | Date }>({
  tableHeader,
  rowData,
  renderRow,
  title,
  ...
  return tableHeader
    ?.filter(col => col.id !== "actions" && col.id !== "action" && col.id !== "checkbox" && col.value)
    .map(col => col.value || col.id)
  ...
  const uniqueValues = useMemo(() => {
    ...
    return filterOptionsData[selectedFilterField] || [];
  }, [selectedFilterField, filterOptionsData]);

  // Handle filter field selection
  const handleFieldSelect = useCallback((field: string | null) => {
    if (onFilterFieldSelect) {
      onFilterFieldSelect(field);
    }
  }, [onFilterFieldSelect]);
  ...
  }, [onFiltersChange, setCurrentFilterState]);
  ...
  const handleClearSearchAndDate = useCallback(() => {
    ...
    if (setCurrentFilterState) {
      setCurrentFilterState((prev: any) => ({
        ...
        dateRange: { start: null, end: null },
        page: 1,
        filters: {}, // Also clear filters
      }));
    }
    
    // Clear filter state as well
    handleFiltersChange({});
  }, [setCurrentFilterState, handleFiltersChange]);
  ...
  // Check if lastStatusChangeDate date is older than 3 days
  ...
  const handleExcelDownload = useCallback(() => {
    const headers = excelHeaders
      ? excelHeaders
      : tableHeader
        .filter((col) => col.id !== "checkbox" && col.id !== "action" && col.id !== "actions")
        .map((col) => col.label);
    const data = excelData
      ...
      : rowData.map((row) => {
          ...
          tableHeader
            .filter((col) => col.id !== "checkbox" && col.id !== "action" && col.id !== "actions")
            .forEach((col) => {
              const value = (row as any)[col.id] ?? "N/A";
  ...
  }, [excelHeaders, excelData, rowData, tableHeader, title]);
  ...
  const getPaginationItems = () => {
    ...
    if (page < pageCount - 4 && pageCount > 5) {
      items.push(
            ...
            {showFillter && (
              <Box>
                <FilterDropdown
                  ...
                  defaultFilter={defaultFilter || currentFilterState}
                />
              </Box>
            )}
                  ...
                  <React.Fragment key={row?.id || row?._id || index}>
                    ...
                    >
                      ...
                      {renderExpandedRow && (
                        <TableCell>
                          <IconButton
                            onClick={() => toggleExpandRow(row?.id || (row as any)._id || index.toString())}
                            size="small"
                            sx={{ padding: 0 }}
                          >
                            {expandedRowId === (row?.id || (row as any)._id || index.toString()) ? (
                              <FaChevronUp size={14} />
                            ) : (
                    ...
                    {renderExpandedRow && expandedRowId === (row?.id || (row as any)._id || index.toString()) && (
                      <TableRow>
                        <TableCell
                        ...
                        >
                          <Collapse in={expandedRowId === (row?.id || (row as any)._id || index.toString())} timeout="auto" unmountOnExit>
                            <Box sx={{ p: 2 }}>
