...
//   const handleSourceSelect = async (source: 'godown' | 'factory' | 'new') => {
//     // Require inventory selection before confirm (if applicable)
//     if ((source === 'godown' || source === 'factory') && !selectedBoxIds.length) {
//       Swal.fire({
//         icon: 'warning',
//         title: 'Please Select Inventory',
//         text: `You must select one or more ${source === 'godown' ? 'Godown' : 'Factory'} batches before proceeding.`,
//         confirmButtonColor: '#1976d2',
//       });
//       return;
//     }
...
//       // ✅ Include all selectedBoxIds in API data
//       const payload: any = { step: newStep };
//       if (selectedBoxIds.length) payload.inventories = selectedBoxIds;

//       await dispatch(updateQPOrderThunk({ id: formData._id, data: payload })).unwrap();
...
//               <FormControl fullWidth size="small" sx={{ mb: 2 }}>
//                 <InputLabel>
//                   {source === 'godown' ? 'Select Available Batches' : 'Select Factory Batches'}
//                 </InputLabel>
//                 <Select
//                   multiple
//                   label={source === 'godown' ? 'Select Available Batches' : 'Select Factory Batches'}
//                   value={selectedBoxIds}
//                   onChange={(e) => setSelectedBoxIds(e.target.value as string[])}
//                   renderValue={(selected) => (
//                     <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
//                       {(selected as string[]).map((value) => {
//                         const box = list.find((b: any) => b._id === value);
//                         return (
//                           <Chip
//                             key={value}
//                             label={box?.boxName || 'Box'}
//                             size="small"
